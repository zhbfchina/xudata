    ...
    req_plans_config.success = function (jsonString) {
        ...
        if (jsonObj.IsOk == 1) {
            ...
            $.each(Data, function (kD, vD) {
                var htmlData = '';
                htmlData += '<div class="img-list"><a href="../page/models/' + Data[kD].ModelID + '.html?planId=' + Data[kD].PlanID + '" target="_blank" class="img-box"><img class="image" src="' + Data[kD].ImgUrl + '" /></a><div class="img-note">' + Data[kD].PlanName + '</div></div>';
            ...
            //    window.open('page/models/' + $modelId + '.html' + '?planId=' + $planId);//相当于<a href="../../page/planList.html" target="_blank"><img src="img.jpg" /></a>
            //});
        } else {
            alert("接收后台响应的数据失败！");
            return false;

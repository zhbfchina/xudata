...
$(function () {
    ...
    $("div[type=table]").on("click", function () {
        ...
        if ($(this).children(".clear-table-wrap").length == 0) {
            ...
            /*获取newPlanList页面通过URL传过来的参数用户名
            注意：相应的model.html页面必须引用xhutility.js，否则无法调用parseUrl函数
            */
            var v = parseUrl();//解析所有参数
            //alert(v['user_name']);//获取想要参数的值
             login_user_name = v['user_name'];
            /***获取其他页面通过URL传过来的参数，注意要在相应js对应的HTML页面中引用xhutility.js，否则无法调用parseUrl函数***/
            ...
            $.ajax({
                ...
                //url: "http://localhost:16613/web/Data/zhbNewData.txt",
                //dataType: 'text',
                url: "http://192.168.8.108/xhq/SystemApi.ashx?req_type=5&username=" + login_user_name,
                //dataType: "jsonp",
                //contentType: "application/json; charset=utf-8",//发送信息至服务器时内容编码类型,不加的话data可以对象，使用的话data只能是json字符串
                async: false,
...
function Plan(m, pm) {
    ...
    this.Modules = m;
    this.CreateUser = login_user_name;
}
...
function Modules(index, dataCode) {
    ...
    if (mddule_is_history == 0) {//不是历史数据，即是实时数据
        config = new Config(mddule_is_history, module_rt_config_interval, 0, module_rt_config_threshold);//调用函数Config，即，每个图表的配置信息       
    }
    else {
        config = new Config(mddule_is_history, 0, module_rt_config_interval, module_rt_config_threshold);
    }
...
//★每个图表的配置信息
function Config(IsHistoryC, RefreshTimeC, HistoryDayGapC, LimitValueC) {
    this.IsHistory = IsHistoryC;
    this.RefreshTime = RefreshTimeC;

...
$(function () {
    ...
    $("input[type=submit]").on("click", function () {
        var postData = "";
        postData = JSON.stringify(dic);//json对象转json字符串：JSON.stringify(jsonObj);
        alert(postData);
...
//function history_configPara(a, b,c) {
//    module_rt_config_interval = a;
//    module_rt_config_threshold = b;
//    mddule_is_history = c;
//}
...
function setIntervalSetEchart() {
    ...
    (function (tg, option) {
        ...
        var intervalId = setInterval(function setEchart() {//★★★★★★注意：不需要设置形参，因为子页面调用此函数时就没有传参数
            ...
            bindClearEvent(intervalId, clearID, setTable, op);
        ...
        //调用清除绑定“清除”的点击事件 ★★★在自执行函数内，计时器外，调用自定义的bindClearEvent事件，目的：在计时器未启动前也可以点击“清除”，清除内容
        bindClearEvent(intervalId, clearID, setTable, option);
    ...
    function bindClearEvent(timeID, cid, st, option) {
        ...
        $("#" + cid).on("click", function (evt) {
            ...
            //取消事件冒泡
            evt.stopPropagation();
            delete dic[cid.substring(5)];//清楚ID对应字符串
        });

    }


    ////自定义函数：移除地图中的放大缩小功能   已经通过修改leaflet里option的属性值（mapOption.zoomControl = false;）实现了移除地图中的放大缩小功能
    //function removeLeafletCss() {
...
//讨论的（待做）：
//var user;
/*var arr = {"PlanID":"","PlanName":"获取输入的标题名称","ModelID":null,"CreateUser":"","CreateTime":"","Modules":[{"ModuleNumber":0,"DataCode":"00000000-0000-0000-0000-000000000000","Config":{"IsHistory":false,"RefrashTime":0,"HistoryDayGap":0,"LimitValue":null}},
...
function ConcatStr(index) {
    var config;
    var model;
    if (mddule_is_history) {
    ...
    model = new Modules(index, "", config);//调用函数Modules，即，每个图表所包含的自身信息和配置信息
    var plan = new Plan(model, index);
    //将index和plan
    dic[index] = plan;
    // dic.push(plan);//push() 方法可向数组的末尾添加一个或多个元素，并返回新的长度。
}
//每个图表所包含的自身信息和配置信息    
function Modules(Mnumber, DCode, Cfig)//DataCode未确定
{
